...
function PhotoUploadSlots({
  ...
  return (
    <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${columns}, ${slotWidth}px)` }}>
      ...
      {photos.map((url, idx) => (
        <div key={idx} className="relative rounded-xl overflow-hidden border border-orange-100" style={{ width: slotWidth, height: slotHeight }}>
      ...
      {Array.from({ length: emptySlots }).map((_, idx) => (
        <div key={`empty-${idx}`} style={{ width: slotWidth, height: slotHeight }} className="rounded-xl border-2 border-dashed border-orange-100 bg-orange-50" />
      ))}
    </div>
  );
}
...
function PhotoUploadHScroll({
  ...
  return (
    <div className="flex flex-wrap gap-3">
      ...
      {photos.map((url, idx) => (
        <div key={idx} className="relative w-[calc((100%-24px)/3)] rounded-xl overflow-hidden border border-orange-100">
          <div className="pb-[100%]" />
          {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {Array.from({ length: emptySlots }).map((_, i) => (
        <div key={`e${i}`} className="relative w-[calc((100%-24px)/3)] rounded-xl border-2 border-dashed border-orange-100 bg-orange-50">
          <div className="pb-[100%]" />
        </div>
      ))}
    </div>
...
function BookingSummaryCard({ booking, loading }: { booking: BookingDisplayInfo | null; loading: boolean }) {
  if (loading) {
    return (
  ...
  return (
    <div className="bg-white border border-orange-100 rounded-2xl p-5 flex items-center gap-5">
      <div className="relative shrink-0">
        <Avatar initial={booking.sitterInitial} src={booking.sitterProfileImage} size="lg" variant="dark" />
      ...
      <div className="flex-1 min-w-0">
        ...
        {booking.petNames.length > 0 && (
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm text-stone-900 font-medium">{booking.petNames.join(", ")}</span>
          </div>
        )}
      </div>
    </div>
...
function DesktopReviewView({
  ...
  const toggleTag = (tag: string) =>
    setReviewData((prev) => ({ ...prev, tags: prev.tags.includes(tag) ? prev.tags.filter((t) => t !== tag) : [...prev.tags, tag] }));
  ...
  return (
    <div className="flex flex-col gap-5">
            ...
            {QUICK_TAGS.map((tag) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm border transition-all ${
                  reviewData.tags.includes(tag) ? "bg-orange-500 text-white border-orange-500" : "bg-orange-50 text-gray-500 border-orange-100 hover:border-orange-500/40"
                }`}
              >
                {tag}
              </button>
      ...
      <div className="bg-white border border-orange-100 rounded-2xl p-7">
        ...
        <PhotoUploadSlots photos={photos} onAdd={onAddPhoto} onRemove={onRemovePhoto} maxPhotos={MAX_REVIEW_PHOTOS} columns={4} slotWidth={129} slotHeight={136} />
    ...
    <div className="flex flex-col gap-4 pb-39">
      {bookingLoading ? (
        ...
        <div className="bg-orange-50 rounded-xl p-3 flex items-center gap-3 border border-orange-100">
          <Avatar initial={booking.sitterInitial} src={booking.sitterProfileImage} size="md" variant="dark" />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5">
              <span className="text-sm font-semibold text-stone-900">{booking.sitterName}</span>
              {booking.serviceType && (
                <span className="text-xs px-2 py-0.5 bg-white border border-orange-100 rounded-full text-orange-500 font-medium">{booking.serviceType}</span>
              )}
            </div>
  ...
  return (
    <div className="flex flex-col gap-4 pb-50">
      ...
      <div className="bg-white border border-orange-100 rounded-2xl p-5">
        ...
        <PhotoUploadHScroll photos={photos} onAdd={onAddPhoto} onRemove={onRemovePhoto} maxPhotos={MAX_REVIEW_PHOTOS} />
...
function ReviewWriteContent() {
      ...
      .then(({ data, error }) => {
        if (error) {
          ...
          setBooking({
            ...
            serviceType: data.service_type ? (SERVICE_TYPE_LABELS[data.service_type] ?? data.service_type) : null,
  ...
  const addPhoto = (file: File) => {
    ...
    setPhotoError(null);
    setPhotos((prev) => prev.length < MAX_REVIEW_PHOTOS ? [...prev, { file, previewUrl: URL.createObjectURL(file) }] : prev);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (photos.length > 0) {
      try {
        image_urls = await Promise.all(photos.map((p) => uploadToCloudinary(p.file, "reviews/photos")));
      } catch {
    ...
    if ("error" in result) {
      setSubmitError((result.error as { message: string } | undefined)?.message ?? "후기 등록에 실패했습니다.");
  ...
  return (
    <div className="min-h-screen bg-orange-50">
      ...
      <div className="md:hidden px-5 pt-4">
        {mobileScreen === 1 ? (
          <MobileScreen1
            reviewData={reviewData}
            setReviewData={setReviewData}
            onNext={() => { setMobileScreen(2); window.scrollTo(0, 0); }}
            booking={booking}
            bookingLoading={bookingLoading}
          />
        ) : (
      ...
      <CustomModal
        ...
        onClose={() => { if (!isSubmitting) setShowSubmitModal(false); }}
