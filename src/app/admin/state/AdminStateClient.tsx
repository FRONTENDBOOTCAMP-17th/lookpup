...
export default function AdminStateClient({
  ...
  const getSitterUser = (r: Reservation): UserInfo | null => {
    ...
    return Array.isArray(sitter.users)
      ? (sitter.users[0] ?? null)
      : sitter.users;
  };
  ...
  const handleStatusChange = async (
    ...
    if (result.error) {
      setErrors((prev) => ({
        ...prev,
        [reservation.id]: result.error!.message,
      }));
      return;
    }
      ...
      prev.map((r) =>
        r.id === reservation.id
          ? {
              ...
              cancel_reason:
                newStatus === "canceled"
                  ? (cancelReason[reservation.id] ?? null)
                  : r.cancel_reason,
            }
          : r
      )
                    ...
                    <div className="px-5 pb-5 border-t border-stone-100">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <InfoRow
                            label="금액"
                            value={`${reservation.total_price.toLocaleString()}원`}
                          />
                          <InfoRow
                            label="서비스 기간"
                            value={
                            ...
                            <div className="flex flex-col gap-1">
                              {[
                                ...
                                .map((t) => (
                                  ...
                                  >
                                    <span className="w-8 text-gray-400 font-medium shrink-0">
                                      {t.label}
                                    </span>
                              ...
                              {ALL_STATUSES.map((s) => {
                                ...
                                return (
                                  <button
                                    ...
                                    type="button"
                                    onClick={() =>
                                      handleStatusChange(reservation, s)
                                    }
