                        ...
                        <div className="flex flex-col gap-3">
                          {isEditing ? (
                            <>
                              ...
                              <div>
                                ...
                                <textarea
                                  value={editMemo}
                                  onChange={(e) =>
                                    setEditMemo(e.target.value.slice(0, 500))
                                  }
                            ...
                            <>
                              <InfoRow
                                label="현재 상태"
                                value={statusMeta?.label ?? report.status}
                              />
                              {report.handled_at && (
                                <InfoRow
                                  label="처리 일시"
                                  value={format(
                                    new Date(report.handled_at),
                                    "yyyy.MM.dd HH:mm",
                                    { locale: ko },
                                  )}
                                />
                              ...
                              <AdminActions
                                ...
                                onDemote={(id, type) =>
                                  runAction(report.id, `demote-${id}`, () =>
                                    adminDemoteUser(id, type),
                                  )
                                }
                                onCancelRequest={(id) =>
                                  runAction(report.id, `cancel-${id}`, () =>
                                    adminCancelRequest(id),
                                  )
                                }
                                onDeactivateService={(id) =>
                                  runAction(report.id, `deactivate-${id}`, () =>
                                    adminDeactivateService(id),
                                  )
                                }
                                onSuspend={(id, type, until) =>
                                  runAction(report.id, `suspend-${id}`, () =>
                                    adminSuspendUser(id, type, until),
                                  )
                                }
                                onUnsuspend={(id, type) =>
                                  runAction(report.id, `unsuspend-${id}`, () =>
                                    adminUnsuspendUser(id, type),
                                  )
                                }
                              />
        ...
        {isUser && (
          <>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-2 pl-6">
                ...
                {actionResults[suspendKey] && (
                  <span
                    className={`text-xs font-medium ${actionResults[suspendKey] === "완료" ? "text-green-600" : "text-red-500"}`}
                  >
                    {actionResults[suspendKey]}
                  </span>
                )}
            ...
            <ActionButton
              ...
              onClick={() =>
                onUnsuspend(target_id, target_type as "user" | "sitter")
              }
              color="orange"
            />
            ...
            <ActionButton
              ...
              onClick={() =>
                onDemote(target_id, target_type as "user" | "sitter")
              }
              color="red"
            />
          </>
...
function ActionButton({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {result && (
        <span
          className={`text-xs font-medium ${isDone ? "text-green-600" : isError ? "text-red-500" : "text-gray-500"}`}
        >
          {result}
        </span>
      )}
...
function InfoRow({
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-xs font-semibold text-gray-400 shrink-0 mt-0.5 w-20">
        {label}
      </span>
      <span
        className={`text-sm text-stone-700 break-all ${mono ? "font-mono text-xs" : ""}`}
      >
        {value}
      </span>
    </div>
